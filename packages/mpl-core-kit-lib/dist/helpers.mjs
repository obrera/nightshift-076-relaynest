import { Qa as getCollectionV1Decoder, ga as MPL_CORE_PROGRAM_ADDRESS, jo as PluginType, oo as getAssetV1Decoder, os as Key } from "./instructions-COrDW8gH.mjs";
import "./generated.mjs";
import { n as lowercaseFirstLetter, t as capitalizeFirstLetter } from "./utils-D2Jm8keN.mjs";
...
function isAssetOwner(addr, asset) {
	return address(addr) === asset.owner;
}
/**
...
async function fetchAssetsByCollection(rpc, collection) {
	...
	const response = await rpc.getProgramAccounts(MPL_CORE_PROGRAM_ADDRESS, {
		encoding: "base64",
		filters: [{ memcmp: {
...
function assetPluginKeyFromType(pluginType) {
	return lowercaseFirstLetter(PluginType[pluginType]);
}
/**
...
function pluginTypeFromAssetPluginKey(key) {
	return PluginType[capitalizeFirstLetter(key)];
}

//#endregion
export { assetPluginKeyFromType, checkPluginAuthorities, collectionAddress, deriveAssetPlugins, fetchAssetsByCollection, fetchAssetsByOwner, fetchAssetsByUpdateAuthority, fetchCollectionsByUpdateAuthority, hasAssetUpdateAuthority, hasCollectionUpdateAuthority, hasPluginAddressAuthority, hasPluginOwnerAuthority, hasPluginUpdateAuthority, isAssetOwner, isFrozen, pluginTypeFromAssetPluginKey };
